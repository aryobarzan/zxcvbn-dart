...
check_matches(
  ...
  bool is_equal_len_args =
      pattern_names.length == patterns.length &&
...
void main() {
  test('matching utils', () {
    ...
    expect(
      lst,
      [1, 2, 3],
      reason: "extending a list with another adds each of the other's elements",
    );
    final lst1 = [1];
    final lst2 = [2];
    ...
    final m1 = PasswordMatch(pattern: 'test', i: 5, j: 5, token: '');
    final m2 = PasswordMatch(pattern: 'test', i: 6, j: 7, token: '');
    final m3 = PasswordMatch(pattern: 'test', i: 2, j: 5, token: '');
    final m4 = PasswordMatch(pattern: 'test', i: 0, j: 0, token: '');
    final m5 = PasswordMatch(pattern: 'test', i: 2, j: 3, token: '');
    final m6 = PasswordMatch(pattern: 'test', i: 0, j: 3, token: '');

    final msg = "matches are sorted on i index primary, j secondary";
    expect(matching.sorted([m1, m2, m3, m4, m5, m6]), [
      m4,
      m6,
      m5,
      m3,
  ...
  test('dictionary matching', () {
    ...
    final dm = (pw) =>
        matching.dictionary_match(pw, ranked_dictionaries: test_dicts);
  ...
  test('reverse dictionary matching', () {
    ...
    final matches = matching.reverse_dictionary_match(
      password,
  ...
  test('l33t matching', () {
    ...
    for (final testCase in [
      ...
      expect(
        matching.relevant_l33t_subtable(pw as String, test_table),
        expected,
        reason: msg,
      );
    }
    ...
    final lm = (pw) => matching.l33t_match(
      pw,
      ranked_dictionaries: dicts,
      l33t_table: test_table,
    );
  ...
  test('spatial matching', () {
    ...
    List<PasswordMatch> matches = matching.spatial_match(
      "rz!${pattern}%z",
      _graphs,
    );
    String msg =
