...
final truncate_float = (double float) =>
    (float / EPSILON).roundToDouble() * EPSILON;
...
void main() {
  test('nCk', () {
    ...
    for (final i in cases) {
      ...
      final k = i[1];
      final result = i[2];
      expect(nCk(n, k), result); //, "nCk(#{n}, #{k}) == #{result}"
    }
    final n = 49;
    final k = 12;
  ...
  test('search', () {
    final m = (int i, int j, double guesses) =>
        PasswordMatch(pattern: 'test', i: i, j: j, token: '', guesses: guesses);
    ...
    dynamic result = scoring.most_guessable_match_sequence(
      password,
      [],
  ...
  test('regex guesses', () {
    PasswordMatch match = PasswordMatch(
      ...
      regex_match: RegExp(r'\w+').firstMatch('aizocdk'),
    );
    ...
    match = PasswordMatch(
      ...
      regex_match: RegExp(r'\w+').firstMatch('ag7C8'),
    );
    ...
    match = PasswordMatch(
      ...
      token: '2005',
      regex_name: 'recent_year',
      regex_match: RegExp(r'\d+').firstMatch('2005'),
    );
    msg = "guesses of MIN_YEAR_SPACE for a year close to REFERENCE_YEAR";
    // The actual value depends on the current year, so test that it's at least MIN_YEAR_SPACE
    final guesses = scoring.regex_guesses(match)!;
    expect(
      guesses >= scoring.MIN_YEAR_SPACE,
      true,
      reason:
          "$msg (got $guesses, MIN_YEAR_SPACE is ${scoring.MIN_YEAR_SPACE})",
    );
  });
  ...
  test('uppercase variants', () {
    for (final testCase in [
      ...
      PasswordMatch m = PasswordMatch(
        pattern: 'test',
        i: 0,
        j: (word as String).isEmpty ? 0 : word.length - 1,
        token: word,
      );
